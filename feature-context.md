...
| `frontend/src/layouts/student-layout.tsx` | **Modified** (Step 15): added `Brain` icon import, inserted a topbar button + mobile menu link between `Courses` and `HP System`/`Cohorts` pointing at `/student/review/dashboard`. Desktop uses the active-state gradient pattern; mobile uses the simpler justify-start variant. Active-state check on `/student/review` so both `/review` and `/review/dashboard` highlight the nav |
...
- [x] ✅ **Keyboard shortcuts + accessibility pass (E3 + E6 — 2026-07-04)**
  ...
  - **`frontend/src/layouts/student-layout.tsx`** — `aria-current="page"`
    on the **Review** nav links (desktop + mobile), computed from
    `isActive('/student/review')`. Pre-existing nav links (Courses,
    Announcements, etc.) don't have this — adding it across all links is
...
- [x] ✅ **Sidebar / topbar nav links (Step 15 — 2026-07-04)** — students can now
  reach `/student/review/dashboard` from the topbar.
  - `frontend/src/layouts/student-layout.tsx` — added `Brain` to the
    `lucide-react` import (line 11), inserted a **desktop topbar button**
    between `Courses` and `HP System` (lines ~249-258) with the same
    active-state gradient pattern used by `Courses`/`Announcements`, plus a
    **mobile menu link** between `Courses` and `Cohorts` (lines ~439-441)
    using the simpler mobile pattern. Both point at
    `/student/review/dashboard` — the dashboard is the canonical landing
    page for the feature; from there the user clicks `Start review` to
    enter the session screen.
  - Active-state uses `isActive('/student/review')` so visiting
    `/student/review` OR `/student/review/dashboard` both highlight the nav
    item (relies on the existing `pathname.startsWith(path + '/')` helper
    in `student-layout.tsx`).
  - `npx tsc --noEmit` exits clean (Step 15 verification 2026-07-04)
...
- [x] ✅ **Review session screen (Step 13 — 2026-07-04)** — student-facing UI to
  ...
  - **Sidebar nav link intentionally NOT added in this step** — the spec puts
    review under `/student/review`; linking can happen in a polish step once the
    Retention Dashboard (Step 14) exists to give the link somewhere meaningful to
    point from
